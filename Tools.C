...
/** 
 * builds a 64-bit long out of an array of 8 bytes ALEX HARTTREE!
 *
...
{
// returns bits low through high, reminder for 1st byte is bits 0-3 then 4-7 and so on 
   if (low < 0 || high > 63 || low > 63 || high < 0)
      return 0;
...
{
   if (low > high || high < low || high > 63 || low < 0)
   {
      return source;
   }
   uint64_t mask = getBits(0xFFFFFFFFFFFFFFFF, low, high);
                         ...
                         int32_t srclow, int32_t dstlow, int32_t length)
{
   ...
   uint64_t ret = clearBits(dest, dstlow, dstlow + (length - 1));
   if((dstlow + length) > 64 || srclow < 0 || length < 0 || dstlow < 0) 
   {
      return dest;
   }
   temp <<= (dstlow);
...
uint64_t Tools::setByte(uint64_t source, int32_t byteNum)
{
   ...
   int front = (byteNum * 8);
   return setBits(source, front, (front + 7));
}
...
bool Tools::addOverflow(uint64_t op1, uint64_t op2)
{
    if (sign(op1) == 0 && sign(op2) == 0) 
      return (sign(op1 + op2) == 1);
    if (sign(op1) == 1 && sign(op2) == 1) 
      return (sign(op1 + op2) == 0);
    return false;
}
...
{
   if (sign(op1) == 0 && sign(op2) == 1) 
      return (sign(op2 - op1) == 0);
   if (sign(op1) == 1 && sign(op2) == 0) 
      return (sign(op2 - op1) == 1);
    return false;
}
